...
function AlipaySubmit(alipay_config){
    ...
    this.alipay_gateway_new = alipay_config.gateway || 'http://wappaygw.alipay.com/service/rest.htm?';
...
AlipaySubmit.prototype.buildRequestMysign = function(para_sort){
    ...
    // console.log('before createLinkstring');
...
//get sign
AlipaySubmit.prototype.parseResponse = function (str_text) {
    
    var pos, sign;
    if (str_text) {
        data = str_text;
        startstr = "<request_token>";
        endstr = "</request_token>";
        startpos = data.indexOf(startstr);
        endpos = data.indexOf(endstr);

        sign = data.substr(startpos+ startstr.length, endpos-startpos-startstr.length);
...
AlipaySubmit.prototype.buildRequestHttp = function (para_temp, callback) {
    ...
    console.log('buildRequestPara finished begin getHttpResponsePOST');
    //console.log('2222');
    // console.log(request_data);
    // console.log(this.alipay_gateway_new);
    // console.log(this.alipay_config['cacert']);
    // console.log(this.alipay_config['input_charset'].toLowerCase().trim());
    //process.exit(0);
    //远程获取数据
