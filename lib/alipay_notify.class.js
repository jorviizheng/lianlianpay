...
AlipayNotify.prototype.verifyNotify = function(_POST, callback){
    ...
    if(Object.keys(_POST).length == 0) {//判断POST来的数组是否为空
        ...
        return false;
    }else if(_POST['notify_data']){
        var notify_data = _POST['notify_data'];
        var parser = new xml2js.Parser();
        parser.parseString(notify_data, function(err, notify_data_js) {
            //生成签名结果
            var isSign = self.getSignVeryfy( _POST, _POST["sign"],false);
            //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
            var responseTxt = 'true';
            
            if (null != notify_data_js.notify["notify_id"][0]) {
                
                self.getResponse(notify_data_js.notify["notify_id"][0], function(result){
                    responseTxt = result;
                    console.log("in notify.class.js verifyNotify(sign==mysign?):", isSign);
                    console.log('is mobile vertify................................');
                    console.log("responseTxt:", responseTxt);
                    //验证
                    //responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
                    //isSign的结果不是true，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
                    //if (responseTxt == 'true' && isSign)  //preg_match("/true/i",responseTxt)
                    console.log('responseTxt type:'+typeof(responseTxt)+'------------------------------');
                    if (responseTxt == 'true')  //BUG for temp use
                        return callback && callback (null,{"out_trade_no":notify_data_js.notify['out_trade_no'][0], "trade_no":notify_data_js.notify['trade_no'][0], "result":"TRADE_SUCCESS"});                        
                    else                         
        ...
        if(_POST['notify_id']){
            self.getResponse(_POST['notify_id'], function(result){
                console.log('is pc vertify................................');
                ...
                //isSign的结果不是true，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
                //if (responseTxt == 'true' && isSign)  //preg_match("/true/i",responseTxt)
                if (result == 'true' )  //BUG for temp use
                    return callback && callback (null,{"out_trade_no":_POST['out_trade_no'], "trade_no":_POST['trade_no'], "result":"TRADE_SUCCESS"});                        
                else                         
                    return callback && callback ({err:false});                                           
            });
        }
...
AlipayNotify.prototype.verifyReturn = function(_GET,cbf){
    // console.log("verifyReturn22");
    if(Object.keys(_GET).length == 0) {//判断GET来的数组是否为空
        cbf(false);
        return false;
    }
    else{
        ...
        if (null != _GET["notify_id"]) {
            this.getResponse(_GET["notify_id"],function(responseText){
                console.log('-------------------------------------------##########################');
                console.log(responseText);
                if(responseText == 'true' && isSign){
                    cbf(true);
                    //return true;
                }else{
                    cbf(false);
        ...
        // console.log("in notify.class.js verifyReturn", isSign);
        //验证
        //responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
        //isSign的结果不是true，与安全校验码、请求时的参数格式（如：带自定义参数等）、编码格式有关
        // console.log('isSing:--------------------'+isSign);
        // console.log('responseTxt:---------------'+responseTxt);
 ...
 * @return 签名验证结果
 */
AlipayNotify.prototype.getSignVeryfy = function(para_temp, sign, sort){
...
AlipayNotify.prototype.verify_my = function(notify_id, callback){
}
/**
